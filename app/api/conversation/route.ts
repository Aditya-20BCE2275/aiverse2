...
const instructionMessage = {
  role: "system",
  content: "You are a conversational agent. Please respond to user messages in a helpful manner.",
};
...
export async function POST(req: Request) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text(); // Log the raw response text for debugging
